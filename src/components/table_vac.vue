<template>
    <el-table :data="data.tableData" v-loading="data.loading" stripe show-summary :summary-method="getSummaries"
        ...
        <el-table-column prop="three" label="接种三针">
            <template #default="scope">
                <div>
                    <el-link @click="detailsPush(props.table_usertype, scope.row.name, `three`)" target="_blank"
                        type="primary">
                        {{ scope.row.three }}
                    </el-link>
                </div>
        ...
        <el-table-column prop="other" label="其他">
            <template #default="scope">
                <div>
                    <el-link @click="detailsPush(props.table_usertype, scope.row.name, `other`)" target="_blank"
                        type="primary">
                        {{ scope.row.other }}
                    </el-link>
                </div>
...
import { pullTableDataVac } from '../api/tabledata'
...
pullTableDataVac(props.table_usertype)
    .then(res => {
